...
%% 2. CONTROL PARAMETERS  (step-function torques, fixed at literature maxima)
%
%  Torque profile: STEP FUNCTION applied from t=0 until liftoff.
%    tau_i(t) = tau_sign_i * tau_max_i   for all t in [0, t_liftoff]
%
%  tau_max fixed at literature isometric maxima:
%    ankle : 175 N.m  (Hussain & Frey-Law, J Foot Ankle Res, 2016)
%    knee  : 247 N.m  (Harbo et al., Eur J Appl Physiol, 2012)
%    hip   : 175 N.m  (Harbo et al., Eur J Appl Physiol, 2012)
%
%  tau_sign = [-1; -1; +1]  (ankle plantarflexion, knee extension, hip extension)
%  The only optimised parameter is theta0 (3 starting joint angles).

ctrl.tau_max = [175; 247; 175];   % N.m  -- literature maxima
...
% Source: optimization_results.csv, BO clean row
%   theta1 (ankle absolute) : -0.6000 rad
%   theta2 (knee relative)  :  1.8000 rad
%   theta3 (hip relative)   : -1.3836 rad
theta0 = [-0.6000; 1.8000; -1.3836];   % optimal angles from BO/ground truth
X0_p1  = [theta0; 0; 0; 0];
...
% Initial torques at t=0 (step function: full torque immediately)
tau_sign = [-1; -1; 1];
tau0 = tau_sign .* ctrl.tau_max;
fprintf('Tau at t=0 (step): [%.2f, %.2f, %.2f] N.m\n', tau0(1), tau0(2), tau0(3));
...
% STEP FUNCTION: full torque applied from t=0 to liftoff.
% t is unused — torque is constant throughout push-off phase.
    tau_sign = [-1; -1; 1];
...
function tau = compute_torques_exp(t,ctrl)  %#ok<INUSL>
% Step function version — matches compute_torques
    tau_sign = [-1;-1;1];
    tau = tau_sign .* ctrl.tau_max;
end