...
def simulate_jump(params_vec, p=None, return_trajectories=False, verbose=False):
    ...
    # Peak-height event: stop when vy_com crosses zero downward (index 6 = vy_com)
    def peak_event(t, X):
        return X[6]
    ...
    # y_com at apex is the last point (solver stopped at vy=0)
    jump_height = float(X2[-1, 1] - y_com_lo)
